---
name: rust-debugging
description: Use when investigating a native Rust crash in the Frameport Android app, decoding tombstones, symbolicating addresses with addr2line, setting up LLDB in Android Studio for the cdylib crates, diagnosing JNI panics, missing logcat output from Rust, RUST_BACKTRACE on Android, or async-Rust debugging via tracing spans. Triggers on "native crash", "JNI panic", "tombstone", "addr2line", "RUST_BACKTRACE", or "lldb-server".
---
...
One `cdylib` crate is loaded via JNI on Android:
- `fuji-ffi` -- camera SDK bridge (loaded as `libfuji_ffi.so`)
...
- `ignore_sigpipe()` -- prevents SIGPIPE kills on socket/fd disconnect

`fuji-ffi` calls these in `JNI_OnLoad`, wrapped in `catch_unwind`.
...
adb logcat -s fuji-core:V fuji-core:V
...
$ADDR2LINE -e rust/fuji-rs/target/aarch64-linux-android/debug/libframeport.so \
    0x12345 0x67890

# Demangle Rust symbols
$ADDR2LINE -Cfe rust/fuji-rs/target/aarch64-linux-android/debug/libframeport.so \
    0x12345
```
...
3. Add symbol search path: `rust/fuji-rs/target/aarch64-linux-android/debug/`
...
pub extern "system" fn JNI_OnLoad(vm: JavaVM, _reserved: *mut c_void) -> jint {
    match std::panic::catch_unwind(|| {
        ...
        init_android_logging("fuji-core");
