...
7. **Do not delegate `unsafe` byte-buffer parsing without explicit alignment context.** Specify the source of bytes ("from PTP-IP network socket, alignment unknown" or "from `Vec<u8>`, 8-byte aligned"). See the `rust-unsafe` skill section on "Pointer reads from untrusted byte buffers".
...
- **`cargo deny` on every PR** — wired in Frameport; do not regress.
...
- `serde_json::to_writer` to a path under app-private storage without an explicit `fsync` call → state loss on LMK SIGKILL.

**Fujifilm protocol-specific:**

- PTP/PTP-IP packet parser that does not validate `length` field before indexing into the byte buffer → out-of-bounds read on malformed camera response.
- BLE payload parser that assumes a fixed offset for a field without checking the advertisement type byte first → wrong data on firmware variations.
- Live-view frame consumer that holds a `Vec<u8>` allocation across an `.await` point in a tight loop → heap churn at 60fps degrades the whole runtime.
...
**Generator + critic pattern**: one model writes the diff, the other reviews it before commit:
...
- `rust-async-internals` skill — cancel-safety annotation discipline, library Drop contracts, async closures.
