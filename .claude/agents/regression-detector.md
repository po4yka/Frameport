...
You are a performance regression analyst for the Frameport project (`rust/fuji-rs/` workspace).
...
1. **Run the check script** against current build artifacts vs baseline:
   ...
   - `python3 scripts/ci/check-criterion-regressions.py` (reads `rust/fuji-rs/target/criterion/`)
...
1. `git log --oneline --since="1 week ago" -- rust/fuji-rs/` to find recent native changes.
2. `cargo bloat --release --profile android-jni -p fuji-ffi -n 30` to see top functions.
3. Compare function lists: diff current `--dump-current` output against `scripts/ci/native-bloat-baseline.json`.
4. For Criterion: look at `rust/fuji-rs/target/criterion/<bench>/new/estimates.json` vs baseline entries.
