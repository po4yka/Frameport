...
## Frameport Module Scope

**Compose module**: `:app` is the only module with the `frameport.android.compose` convention plugin. Scope all `@Composable`, `remember`, `LaunchedEffect`, and UI-related searches to `app/src/main/kotlin/`.

**Key directories**:
- `app/src/main/kotlin/dev/po4yka/frameport/ui/screens/` -- per-feature screen composables
- `app/src/main/kotlin/dev/po4yka/frameport/ui/components/` -- design system components (buttons, cards, inputs, indicators, feedback, navigation, scaffold)
- `app/src/main/kotlin/dev/po4yka/frameport/ui/theme/` -- FrameportTheme, Color, Type, Spacing, Shape, Motion
- `app/src/main/kotlin/dev/po4yka/frameport/ui/navigation/` -- NavHost, BottomNavBar, Route
- `feature/*/src/main/kotlin/` -- feature module composables (onboarding, connection, gallery, import, remote, liveview, settings, diagnostics)

**Non-Compose modules** (do NOT search for Compose UI definitions):
- `:core:model` -- domain models
- `:core:storage` -- DataStore settings
- `:camera:data` -- camera protocol data layer
- `:native:fuji-rust-android` -- Rust JNI bridge
- `:camera:diagnostics` -- connection diagnostics logic
...
For Frameport, the key paths are listed in the module scope section above.
...
- Strong Skipping is on by default at Kotlin **2.0.20+**; Frameport uses Kotlin 2.3.20, so Strong Skipping is active
