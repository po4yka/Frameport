...
- Adding a new crate to `rust/fuji-rs/crates/`.
- Reviewing or modifying `rust/fuji-rs/Cargo.toml` `[workspace.lints.*]` sections.
- Authoring `rust/fuji-rs/clippy.toml`.
...
Place at the workspace root `rust/fuji-rs/Cargo.toml`. Each subcrate inherits via `[lints] workspace = true`.
...
Place at `rust/fuji-rs/clippy.toml`. Configures parameterized lints from the table above.
...
```rust
// Pure-logic crates: zero unsafe allowed. Frameport examples: fuji-ptp,
// fuji-ptpip, fuji-core, fuji-liveview, fuji-ble-protocol.
#![forbid(unsafe_code)]
...
rg '^s*#[allow([^)]+)]' --type rust -n rust/fuji-rs/  # audit existing allows
```
