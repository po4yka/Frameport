...
description: Rust code discipline for the Frameport workspace — API design, anti-patterns, panic policy, errors, RAII, allocation, concurrency, unsafe, and lints.
---

# Rust Discipline -- Frameport
...
Grep for violations: `rg 'fn .+\(&String\|&Vec<\|&PathBuf' rust/fuji-rs/ --type rust -n`
...
Grep for violations: `rg "struct .+<'.+>\s*\{" rust/fuji-rs/ --type rust -n` then check for `&'_ mut` fields.
...
- Add a loom or targeted test for any new publish/subscribe atomic, mirroring the `fuji-diagnostics` scan cancellation pattern.
...
- **`std::thread::spawn`** for long-lived loops or large-ish blocking work that would otherwise starve the blocking pool. The `fuji-transfer` transfer-loop thread is the reference pattern.
...
- The `missing_safety_doc` and `not_unsafe_ptr_arg_deref` workspace-wide `allow`s exist **only for `extern "system"` JNI entry points** in `fuji-ffi` and `fuji-ffi`. Internal `unsafe fn` in non-FFI modules must still carry a `# Safety` rustdoc section.
...
Grep for violations: `rg "fn .+<'[a-z]>.*HashMap.*&'[a-z]" rust/fuji-rs/ --type rust -n`
...
Grep for violations: `rg "^impl<T(:\s*\w[\w:+ ]*)?>\s+\w+\s+for\s+T\b" rust/fuji-rs/ --type rust -n`
...
Grep for violations:
```bash
rg "Box::new\(\s*\[0?[a-z0-9_]+\s*;\s*[0-9]{4,}" rust/fuji-rs/ --type rust -n
rg "fn .* -> \[[a-z0-9_]+\s*;\s*[0-9]{4,}\]" rust/fuji-rs/ --type rust -n
```
