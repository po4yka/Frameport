...
Frameport-specific: AGENTS.md "Flow hot-path discipline" already calls for `WhileSubscribed(5_000)` over `Eagerly` to prevent leaks across config changes — but only when consumers don't synchronously read `.value`. Audit each `stateIn` site for which constraint applies.
