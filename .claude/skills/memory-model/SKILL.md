...
Guide agents through Rust memory ordering on ARM64 Android: orderings, the happens-before relation, atomic operations, fences, and patterns used in the Frameport codebase.
...
No C++ source code. All concurrency is in Rust crates under `rust/fuji-rs/`.

### Crates using atomics

| Crate | File | Types | Orderings | Pattern |
|-------|------|-------|-----------|---------|
| fuji-diagnostics | telemetry.rs | AtomicU64 | Relaxed | Statistics counters |
| fuji-transfer | transfer.rs | AtomicBool | Release/Acquire | Shutdown flag (cross-thread signal) |
| fuji-core | connect.rs | AtomicBool | SeqCst | Test-only assertion flag |
| fuji-diagnostics | diagnostics/runtime.rs | AtomicBool | Acquire/Release at cancellation boundaries | Scan cancel flag |
| fuji-core | tests.rs | AtomicUsize | Relaxed | Test call counters |
| fuji-sim | fixtures/echo.rs | AtomicBool | Relaxed | Stop flags (poll loops) |
...
**2. Shutdown/cancel flag (Release/Acquire)** -- used in fuji-transfer transfer loop:
```rust
