...
**Frameport note:** This project uses explicit `@Immutable`/`@Stable` on all UI model classes and maintains `app/compose-stability.conf`. Expect higher-than-average `skippable%`. Verify continued coverage rather than introducing stability annotations from scratch.
...
- hardcoded magic numbers like `.padding(12.dp)` or `.size(24.sp)` or explicit `Color(0xFF...)` instead of routing through `FrameportThemeTokens` or `MaterialTheme` tokens. A clean theme is vital for dark mode and accessibility -> [theming](https://developer.android.com/develop/ui/compose/designsystems/material3)
