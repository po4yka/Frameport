...
### Primary path -- Frameport convention plugin

The `frameport.android.compose` convention plugin enables Compose Compiler reports when the Gradle property is set. No edits to `build.gradle.kts` are required:
...
Frameport already maintains a stability config at `app/compose-stability.conf` that marks proto types, `java.time.*`, `kotlin.time.Duration`, `kotlinx.collections.immutable.*`, and core data models as stable. Check this file for coverage before recommending new entries.

The config is wired in via the `frameport.android.compose` convention plugin. To add new types, edit `app/compose-stability.conf` directly (one fully-qualified class per line, glob patterns allowed):
...
Frameport already has a `:baselineprofile` module. Verify it is up to date:
...
Strong Skipping is on by default at Kotlin 2.0.20+. Frameport uses Kotlin 2.3.20, so Strong Skipping is active by default.
...
When arriving at the Frameport repo, run these in order before scoring:
...
4. Run the convention plugin build: `./gradlew :app:assembleRelease -Pframeport.composeReports=true --no-daemon`
