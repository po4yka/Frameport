...
description: Android JNI canonical patterns for the Frameport workspace — panic containment, AttachCurrentThread/Detach discipline, local-ref frames, JNIEnv non-Send-through-await, JByteArray-vs-DirectByteBuffer for hot paths, live-view frame callbacks, and transfer-to-fd wiring. Use when authoring or modifying any code under fuji-ffi or any JNI export in the dev.po4yka.frameport.nativebridge package.
---

# Rust Android JNI -- Frameport

## Purpose

Consolidate the five canonical JNI rules that LLM-generated Rust regularly violates on Android targets. The `rust-unsafe` skill covers the general JNI patterns (catch_unwind, JString::from_raw); this skill is the Android-specific extension that adds thread attachment lifetime, local-ref frame management, JNIEnv async-boundary discipline, hot-path data marshaling, and live-view frame callback wiring anchored to the `fuji-ffi` crate.
...
- Reviewing a diff that touches `rust/fuji-rs/crates/fuji-ffi/`.
- Wiring or auditing a live-view frame delivery callback or transfer-to-fd path.
...
env.call_method(&listener, "onFrameReady", "(I)V", &[frame_id.into()])?;
...
for object in &media_objects {
    let s = env.new_string(object.name)?;
...
`env.get_byte_array_region` and `env.get_byte_array_elements` COPY the entire byte array between JVM heap and native memory. For hot-path data (live-view frames, transfer buffers), this throughput-couples the JNI boundary to data rate.

For file transfer specifically: the canonical Frameport pattern is `RawFd` ownership transfer at session start. The fd crosses JNI exactly ONCE as a `jint`. Rust then reads/writes the destination file or socket directly; media bytes never cross the JNI boundary again.

If you must transit bytes through JNI (control-plane only, NOT hot path):

```rust
// Acceptable for control-plane (e.g., PTP command response):
...
## Live-view frame callback wiring

Live-view frames arrive from the camera at high rate. Two delivery patterns both satisfy the hot-path rule.

### Option A — fd ownership transfer (preferred for transfer and live-view sockets)

Kotlin passes a writable file descriptor (pipe write-end or MediaStore fd) to Rust at session start. Rust writes frame data directly to the fd via `AsyncFd`; frame bytes never cross the JNI boundary again.

```rust
fn start_liveview_session(vm: &Arc<JavaVM>, fd: RawFd) -> io::Result<()> {
    // fd ownership transferred; Rust calls AsyncFd::new(fd) and writes to it.
    // Java side reads from the read-end of the pipe.
    Ok(())
}
```

### Option B — Direct JNI callback (for low-rate notifications and control events)

```rust
fn notify_transfer_progress(vm: &Arc<JavaVM>, listener: &GlobalRef, bytes_done: u64) -> jni::errors::Result<()> {
    let _guard = vm.attach_current_thread()?;
    let env = _guard.deref_mut();
    env.call_method(listener.as_obj(), "onProgress", "(J)V", &[bytes_done.into()])?;
    Ok(())
}
```

Cost: `attach_current_thread` is ~5–15 µs on Android. Acceptable for progress notifications (one per second or less); unacceptable for per-frame live-view delivery.
...
let runtime = tokio::runtime::Builder::new_multi_thread()
    ...
    .thread_name_fn(|| {
        ...
        format!("frameport-tokio-{}", N.fetch_add(1, Ordering::Relaxed))
    })
    .enable_all()
    .build()?;
```
...
6. Hot-path data (live-view frames, bulk transfer) does NOT cross JNI as `JByteArray`. Use fd-ownership-transfer or `DirectByteBuffer`.
