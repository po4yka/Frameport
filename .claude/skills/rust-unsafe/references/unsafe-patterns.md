# Unsafe Patterns Reference -- Frameport
...
## JNI macro pattern (fuji-ffi)

The `export_jni!` macro stamps out `#[unsafe(no_mangle)] extern "system"` functions that delegate to an `_entry` function. The entry function handles `with_env`/`Outcome`. When adding a new JNI export, use this macro rather than hand-writing the extern fn.
