...
cd rust/fuji-rs
cargo +nightly careful test -p fuji-ffi --no-fail-fast
cargo +nightly careful test -p fuji-core --no-fail-fast
```
...
Apply to:
- Custom atomic-based publish/subscribe flags (e.g., the `fuji-diagnostics` scan cancellation path, the `fuji-transfer` transfer-loop shutdown flag).
...
Apply to: `fuji-ptp`, `fuji-ptpip`, `fuji-ble-protocol`, `fuji-transfer`, `fuji-liveview`. Any AI-generated parser without a proptest is incomplete.
...
cd rust/fuji-rs/crates/fuji-ptpip
cargo fuzz init
...
# fuzz_target!(|data: &[u8]| {
#     let _ = frameport_vless::wire::parse_header(data);
# });
...
cd rust/fuji-rs
cargo mutants -p fuji-ptpip --jobs 4
