...
2. **`compose-preview` CLI** — ships a Gradle init-script that registers `discoverPreviews` and `renderAllPreviews` and drives them via the Tooling API. Because Frameport declares the plugin in `gradle/libs.versions.toml`, the CLI's auto-inject *skips* its own classpath injection and just supplies the task implementations — the two layers don't fight.
...
Defaults match Frameport's `debug` variant and `targetSdk=37`. The convention plugin (`frameport.android.compose.gradle.kts`) leaves the extension at its defaults — override locally if you need to spot-check a different SDK level.
...
`build-logic/convention/src/main/kotlin/frameport.android.compose.gradle.kts` applies `id("ee.schimke.composeai.preview")` alongside `org.jetbrains.kotlin.plugin.compose`, so every module that opts into `frameport.android.compose` (today: `:app`) participates. Future Compose modules pick it up automatically with no per-module configuration.
