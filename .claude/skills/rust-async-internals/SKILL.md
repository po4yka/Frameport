...
# Rust Async Internals -- Frameport

## Purpose

Guide agents through async patterns specific to Frameport: JNI-to-async bridging,
the transfer-loop event-driven architecture, tokio runtime configuration for Android
NDK, CancellationToken-based shutdown, and common select!/join! pitfalls.

## Triggers

- "Why is the transfer loop blocking / slow?"
...
Frameport has two independent tokio runtimes bridged from JNI:

1. **Session runtime** (`fuji-ffi`): shared `multi_thread(2)` runtime
   stored in a `OnceCell<Arc<Runtime>>`. A dedicated `std::thread` calls
   `runtime.block_on(run_camera_session(...))` so the JNI thread is not blocked.

2. **Control runtime** (`fuji-ffi`): `run_camera_with_embedded_control` blocks
...
```rust
// fuji-ffi/src/session/lifecycle.rs -- the canonical pattern
//
...
let worker = std::thread::Builder::new()
    ...
    .spawn(move || {
        let result = std::panic::catch_unwind(AssertUnwindSafe(|| {
            runtime.block_on(run_camera_session(config, fd, cancel, stats))
        }));
...
- Duplicate file descriptors (`nix::unistd::dup`) before passing to async --
  Android can revoke the original fd at any time.
...
- **≥ 1.42.1** — fixes a `broadcast::Sender::clone()` soundness bug (missing synchronization for `Send + !Sync` payloads) and a `CancellationToken` race where futures that polled to `Ready` before the token fired were not cancelled. Frameport's connection-level abort paths rely on the cancellation fix. See the tokio [CHANGELOG](https://github.com/tokio-rs/tokio/blob/master/tokio/CHANGELOG.md) and [PR #7462](https://github.com/tokio-rs/tokio/pull/7462).
- **≥ 1.51.1** — fixes a file-descriptor leak when an `io_uring` `open` operation is cancelled before completion. `fuji-core` cancels in-flight FS/IO operations on session teardown; below this version the leaked fds accumulate until the process exits. See [PR #7983](https://github.com/tokio-rs/tokio/pull/7983).

If `cargo tree -i tokio` shows a version below the floor, promote it in `rust/fuji-rs/Cargo.toml` workspace dependencies (never downgrade a transitive dep to work around a breaking change — open an upstream issue instead).

### Runtime configuration for Android NDK

```rust
// fuji-ffi/src/session/registry.rs
tokio::runtime::Builder::new_multi_thread()
    .worker_threads(2)          // constrained for mobile battery/CPU
    .thread_stack_size(1024 * 1024) // 1 MiB -- Android default is small
    .thread_name("fuji-core")
    .enable_all()
    .build()
```
...
## transfer-loop event-driven architecture

The core transfer loop runs a single-task 6-phase loop (`transfer_loop_task` in
`fuji-transfer/src/transfer_loop.rs`):

1. **Poll USB/BLE fd** -- read PTP event packets via `AsyncFd::try_io`, classify
2. **PTP-stack poll** -- advance PTP state machines in userspace
3. **New sessions** -- detect ready camera objects, spawn `PtpSession` tasks
4. **Transfer bridge** -- pump data between PTP-stack channels and session tasks
5. **Flush tx_queue** -- write PTP-stack packets back to USB/BLE fd
6. **Wait** -- `select!` on USB/BLE readable / PTP-stack timer / transfer events / cancel
...
Frameport uses `tokio_util::sync::CancellationToken` (not `tokio::sync::Notify`)
for structured shutdown. The camera session state machine transitions through
...
The following async-Rust pitfalls have their own catalog page so this skill stays focused on Frameport's architecture. Read [references/async-pitfall-catalog.md](references/async-pitfall-catalog.md) when authoring or reviewing async code touched by any of them:
